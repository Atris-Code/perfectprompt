...
const ThermalView: React.FC<{ reactor: ReactorState, heatingTime?: number, coolingTime?: number }> = ({ reactor, heatingTime, coolingTime }) => {
    ...
    const barWidth = Math.min(100, (reactor.temperature / 900) * 100);
    ...
    return (
        <>
            ...
            <div className="mt-auto pt-2 border-t border-slate-700 text-xs space-y-1 text-slate-300">
                <div className="flex justify-between"><span>{reactor.feedstockType === 'Grano de Caucho' ? t('view.hyperion.gcrPurity') : t('view.hyperion.pelletPurity')}:</span> <span className="font-mono">{reactor.pelletPurity.toFixed(2)}%</span></div>
...
interface Hyperion9Props {
    ...
    dexListing: DEXListing | null;
    onFleetCommand: (command: FleetCommand) => void;
}

export const Hyperion9: React.FC<Hyperion9Props> = ({ reactors, navigateToHmi, setView, heatingSeconds, coolingSeconds, phoenixSiloLevel, dexListing, onFleetCommand }) => {
    const { t } = useTranslations();
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-8">
                ...
                {dexListing && (
                     <div className="bg-slate-800/50 border border-slate-700 rounded-lg p-4 flex items-center justify-between">
                        <div>
                            <h3 className="text-lg font-bold">Mercado Secundario (DEX)</h3>
                            <p className="text-sm text-cyan-400">{dexListing.tokenName}</p>
                        </div>
                        <div className="text-right">
                            <p className="text-2xl font-mono font-bold">{dexListing.price.toFixed(4)} <span className="text-lg text-slate-400">USDC</span></p>
                            <p className={`text-sm font-semibold ${dexListing.change24h >= 0 ? 'text-green-400' : 'text-red-400'}`}>{dexListing.change24h.toFixed(2)}% (24h)</p>
                        </div>
                    </div>
                )}
            </div>
             ...
             <div className="mb-8 p-4 bg-slate-800/50 border border-slate-700 rounded-lg">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <div className="flex items-center gap-2">
                        ...
                        <button
                            onClick={() => onFleetCommand({ type: 'APPLY_PRESET', payload: selectedPreset })}
